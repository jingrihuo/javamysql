...
public class FrmReCartype extends JDialog implements ActionListener {
	...
	private static Object tblCarTitle[] = new String[] { "车辆车型", "申请状态" };
	...
	static void reloadUserCarbrandApply() throws BaseException {
		try {
			allCartype = SummerWorkUtil.cartypeManager.loadAllApplyCartypes();
		...
		for (int i = 0; i < allCartype.size(); i++) {
			...
			tblCarData[i][1] = allCartype.get(i).getCartypetype();
		}
	...
	public FrmReCartype() throws BaseException {
		super();
		...
		this.dataTableCar.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				UserCartype= allCartype.get(FrmReCartype.this.dataTableCar.getSelectedRow());
			}
		});
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == this.PassApply) {
			...
			if (JOptionPane.showConfirmDialog(this, "确定拒绝申请嘛？", "确认",
					JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
				try {
					SummerWorkUtil.cartypeManager.ModifyCartype("拒绝",allCartype.get(i));
					this.reloadUserCarbrandApply();
				} catch (BaseException e1) {
					e1.printStackTrace();
				}
